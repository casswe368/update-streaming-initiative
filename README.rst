...
Major update as-of Dec 28, 2020. AO3 announced today that because of the flash player being decomissioned they will be `updating all current embedded audio that uses the Dewplayer <https://archiveofourown.org/admin_posts/18129>`_ (AO3's flash player for audio) in the "near future." No details on when that is, but it may be worth it to wait for AO3 to update the code and then just check for embedded streaming that uses other flash players. 
